...
std::optional<UnitClass> unitClassFromString(const std::string& name) {
    static const std::unordered_map<std::string, UnitClass> lookup = {
        ...
        {"FrontierEngineer", UnitClass::FrontierEngineer},
        {"MessengerCavalry", UnitClass::MessengerCavalry},
        {"FrontierRanger", UnitClass::FrontierRanger},
        {"BannerBearer", UnitClass::BannerBearer},
        {"BattleMage", UnitClass::BattleMage},
        ...
        {"FortGarrisonCaptain", UnitClass::FortGarrisonCaptain},
        {"FrontierBeast", UnitClass::FrontierBeast},
    };
...
std::optional<GameData> loadGameData(const std::string& dataDir) {
    ...
    for (const auto& s : (*regionsJson)["stages"]) {
        ...
        try {
            ...
            if (s.contains("primarySecureTileAlternative")) {
                const auto& h = s.at("primarySecureTileAlternative");
                stage.primarySecureTileAlternative = StageContentData::HoldTileMissionRuleData{
                    h.at("id").get<std::string>(), h.value("requiredHoldRounds", 2), h.value("zoneMinCol", 0),
                    h.value("zoneMaxCol", kGridCols - 1)};
            }
