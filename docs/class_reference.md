...
Heavy Infantry's and Frontier Engineer's Class/weapon/innate ability/skills are
implemented (M7項目1); join path (party recruitment) remains out of scope.
...
後半6兵種のうち重装兵と辺境工兵はClass・武器・固有能力・スキルを実装済み(加入経路は
対象外)。残り4兵種は未実装だが、実装時の基準値と固有能力を次で固定する。数値は初期6
兵種と同じ装備補正前の基礎値であり、通常レベルによって上昇しない。
