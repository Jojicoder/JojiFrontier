...
Heavy Infantry's, Frontier Engineer's, and Messenger Cavalry's Class/weapon/
innate ability/skills are implemented (M7項目1); join path (party recruitment)
remains out of scope. Banner Bearer, Frontier Ranger, and Battle Mage remain
...
後半6兵種のうち重装兵・辺境工兵・伝令騎兵はClass・武器・固有能力・スキルを実装済み
(加入経路は対象外)。残り3兵種は未実装だが、実装時の基準値と固有能力を次で固定する。
数値は初期6兵種と同じ装備補正前の基礎値であり、通常レベルによって上昇しない。
