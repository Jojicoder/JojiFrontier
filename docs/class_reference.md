...
|---|---|---:|---:|---:|---:|---:|---:|---:|---|
| `MarchCaptain` | 行軍隊長 | 22 | 7 | 1 | 7 | 5 | 4 | 3 | 鉄の剣、威力5、射程1、物理 |
| `VeteranGuard` | 古参守備兵 | 28 | 8 | 0 | 3 | 8 | 3 | 2 | 鉄のランス、威力6、射程1、物理 |
| `WatchArcher` | 監視弓兵 | 18 | 6 | 0 | 7 | 5 | 2 | 3 | 監視弓、威力5、射程2-3、物理 |
| `FrontierScout` | 辺境斥候 | 18 | 5 | 0 | 9 | 3 | 3 | 5 | 斥候刀、威力4、射程1、物理。常時回避率+10% |
| `Spearman` | 槍兵 | 23 | 7 | 0 | 5 | 7 | 3 | 3 | 鉄の槍、威力6、射程1-2、物理 |
| `DawnChirurgeon` | 暁の衛生兵 | 17 | 2 | 6 | 6 | 4 | 8 | 3 | 暁の杖、威力3、射程1-2、魔法 |
| `HeavyInfantry` | 重装兵 | 32 | 8 | 0 | 2 | 12 | 2 | 2 | 鉄の大槌、威力7、射程1、物理 |
| `FrontierEngineer` | 辺境工兵 | 21 | 6 | 1 | 5 | 5 | 4 | 3 | 工作槌、威力5、射程1、物理 |
| `MessengerCavalry` | 伝令騎兵 | 22 | 7 | 0 | 9 | 4 | 3 | 6 | 伝令剣、威力5、射程1、物理 |
| `FrontierRanger` | 辺境猟兵 | 20 | 6 | 0 | 7 | 4 | 4 | 3 | 狩猟弓、威力4、射程2、物理 |
| `BannerBearer` | 旗手 | 22 | 5 | 2 | 5 | 5 | 6 | 3 | 戦旗槍、威力4、射程1-2、物理 |
| `BattleMage` | 戦闘魔導士 | 16 | 1 | 9 | 5 | 2 | 7 | 3 | 魔導焦点具、威力6、射程1-2、魔法 |

- `Bandit`など敵専用兵種は12兵種へ含めず、別の敵データとして管理する
- SPDは現行のPhase内行動回数や命中率を増やさない
- 基本必中を維持する。ただし辺境斥候の常時回避率+10%と、茂みなど明示された地形は命中率を変更する
- 辺境斥候の回避補正は敵の命中率を10パーセントポイント下げる。茂みの20ポイントと加算し、通常床では命中90%、茂みでは命中70%になる
- 敵が同じ兵種を使う場合も基礎能力と固有能力を共有する
- 戦闘魔導士の魔法は短射程で、希少性・疲労・非攻城兵器という共有設定を維持する

**DEF調整の記録(2026-07)**: `jf_forest_balance`実測で、狼の通常攻撃力11に対し古参守備兵DEF10が
被ダメージ1(下限)まで抑え込む一方、監視弓兵DEF3/暁の衛生兵DEF2は8-9ダメージを受け、
「古参守備兵抜きだと灰枝の林縁ですら勝率が100%→56.6%まで崩壊する」という偏りが実測で判明した。
古参守備兵DEF10→8、監視弓兵DEF3→5、暁の衛生兵DEF2→4という2段階の調整で、古参守備兵抜きの
生存性を大きく改善した(詳細・実測値は[`campaign_balance.md`](campaign_balance.md)参照)。
3地点連続(無補給)の全滅率がやや目標レンジを超えて残っているが、これは「本来撤退が前提の
最悪ケース」の数値であり許容している。

## Planned expansion roster

All six of Heavy Infantry, Frontier Engineer, Messenger Cavalry, Frontier
Ranger, Banner Bearer, and Battle Mage now have their Class/weapon/innate
ability/skills implemented (M7項目1 complete); join path (party recruitment,
M7項目2) remains out of scope for all six. Ranger's `read_quarry` is a data
flag only (`Unit::quarryRevealed`) - no preview UI yet. Battle Mage is tied to
a rare named specialist and stays out of the general hire pool once M7項目2
lands. Ranger is a tracker/trapper rather than a dedicated monster-hunter;
wolves and large territorial wildlife may still be combat threats in Frontier
scenarios.

## 後半6兵種の基礎仕様

後半6兵種は全6兵種(重装兵・辺境工兵・伝令騎兵・辺境猟兵・旗手・戦闘魔導士)とも
Class・武器・固有能力・スキルを実装済み(加入経路は対象外)。数値は初期6兵種と同じ
装備補正前の基礎値であり、通常レベルによって上昇しない。

| 兵種 | HP | STR | MAG | SPD | DEF | RES | MOV | 兵種固有能力 |
|---|---:|---:|---:|---:|---:|---:|---:|---|
| 重装兵 | 32 | 8 | 0 | 2 | 12 | 2 | 2 | **重量装甲**: ノックバック距離を1減らし、0以下なら移動しない |
| 辺境工兵 | 21 | 6 | 1 | 5 | 5 | 4 | 3 | **野戦工作**: 戦闘中1回、隣接する空きマスへ耐久10の防護板を設置 |
| 伝令騎兵 | 22 | 7 | 0 | 9 | 4 | 3 | 6 | **再移動**: 攻撃・スキル・アイテム後、生存していれば最大2マス移動して行動終了 |
| 辺境猟兵 | 20 | 6 | 0 | 7 | 4 | 4 | 3 | **簡易罠**: 戦闘中1回、隣接する空きマスへ踏むと移動終了する罠を設置 |
| 旗手 | 22 | 5 | 2 | 5 | 5 | 6 | 3 | **戦旗**: マンハッタン距離2以内の味方のSTRとMAGを+1。同効果は重複しない |
| 戦闘魔導士 | 16 | 1 | 9 | 5 | 2 | 7 | 3 | **魔力波及**: 戦闘中1回、通常魔法攻撃後に対象と上下隣接する敵へ固定3ダメージ |
...
| 古参守備兵 | Zone of Controlで敵の移動継続を止め、通路を封鎖する | 味方の被害を肩代わりしない、ノックバック完全無効を標準装備にしない、突撃敵だけへの特効を持たない。射程2以上の攻撃手段を持たず、隣接攻撃に限定する(最高DEFとZone of Controlを持つ兵種が槍兵と同じ間合いまで持つと役割が重複するため)。重装兵・槍兵と分ける |
