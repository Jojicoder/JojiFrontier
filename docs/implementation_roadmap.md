...
- M9 残り8地域: 灰鉄採石場(第3地域)着手。M9-A(骨格5地点+地点1)・M9-B(地点2
  「砕石段丘」)完了、地点3A/3B/4/5は仮プレースホルダーのまま(次Slice以降で
  1地点ずつ本格化)

直近の未完了(優先度順):

1. M9-C以降 灰鉄採石場の残り地点(地点3A「旧採掘坑」から1地点ずつ、M6-B/C方式)
...
`primarySecureTileAlternative`を新規実装)完了(2026-07)。地点3A/3B/4/5は
仮プレースホルダー(汎用敵編成)のまま、M6-B/C方式で1地点ずつ本格化する。
残り7地域は未着手
...
1. **M9-C 灰鉄採石場 地点3A(旧採掘坑)の実コンテンツ化**
   - `docs/regions/ashiron_quarry.md`「3A. 旧採掘坑」どおりの内容
   - M9-Aで新設した`quarry_old_mine`プレースホルダーを置き換え
