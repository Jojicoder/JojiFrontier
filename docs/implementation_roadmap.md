...
Milestone進行(M0〜M5は完了/概ね完了、M8-Aは未実装、詳細は各章参照):

- M6 沈黙した監視所群: M6-A/B/C完了、6地点すべて実コンテンツ化済み。地域締め
  (M6-D、最低保証報酬・4〜6周実測・灰鉄採石場解放)は未着手
- M7 12兵種・仲間・会話: 項目1(後半6兵種)完了。項目2(加入経路)は最小の縦切り
  (加入基盤+重装兵1体)まで完了、残り5兵種は未着手

直近の未完了(優先度順):

1. M6-D: 地域の最低保証報酬、4〜6周の実戦計測、灰鉄採石場解放
2. M7-2: 辺境工兵・伝令騎兵・旗手の加入経路(安全帰還後の候補登録・二重加入防止・Save)
3. M1-E/M9前基盤整理: 残りRegion/Site/Encounter定義化、RewardRule、Route Graph到達検査、
   Objective達成可能性検査
...
状態: **未着手**。M6-A/B/Cで6地点すべてが実コンテンツ化された後に残る、地域単位の
締め作業。

1. 地域の最低保証報酬(6地点通しての資材下限保証、取り逃し分の最終地点回収)
2. 4〜6周の実戦計測(実測して初めてバランス調整の要否を判断する)
3. 灰鉄採石場(次地域)の解放
...
項目2は加入基盤(Pending加入候補→安全帰還で恒久化→加入確定でRoster追加+Tier1スキル
自動装備、受け入れ枠段階)を汎用的に構築し、実際に動く具体例は重装兵(`heavy_recruit`、
表示名「ハドリク」)1体だけを通した。残り5兵種(辺境工兵・伝令騎兵・辺境猟兵・旗手・
戦闘魔導士)の加入条件配線は今後のSliceで`GameApp::confirmRecruitJoin()`のクラス分岐を
1件ずつ拡張する形で追加する(次はM7-2として辺境工兵・伝令騎兵・旗手の3体を予定)。
...
1. **M6-D Cinderwatch締め**
   - 地域の最低保証報酬(6地点通しての資材下限保証、取り逃し分の最終地点回収)
   - 4〜6周の実戦計測
   - 灰鉄採石場(次地域)の解放
   - 各地点の残り保留項目をM7/M4/M10へ正式移管
2. **M7-2 加入経路(Pending加入候補)**
   - 辺境工兵・伝令騎兵・旗手の加入候補配線
   - 安全帰還後の候補登録
   - 二重加入防止
   - Save
...
この3 Sliceが通るまで、他Milestoneの先行実装(M8遠征経済、残り地域など)を並行実装しない。
