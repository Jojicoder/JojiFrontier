...
状態: **地形Profile Slice完了。Encounter・地域・AI・Boss状態のデータ化は未着手。M9着手前の必須基盤**
...
既存実装をこの2点だけ汎用型へ抽出した。予告構造体自体の汎用化は次のBossが実装されるまで保留**
...
による早期勝利防止までは完了。地域Definition側の接続は薬草の沢の採取ルート増援のみ**
...
- **継続課題**: 他地域・他Stageからの`ReinforcementWave`データ接続、増援予告のUI表示、
  中断Save時の`ReinforcementWave`状態の永続化(現状は戦闘状態自体を保存しない既存方針に
  従い、再開時にSeed+Definitionから再生成する想定のまま)
...
状態: **項目3・4(解体・稼働枠モデルの廃止、建設済み施設の常時利用)が完了。
項目1・2(JSON Definition化、旧ID Alias)、項目6(原子的Transaction、部分実装のまま)、
項目8(拠点段階進行・外観変化)は未着手。項目5・7は既存のC++ハードコード
`FacilityNode`レジストリの範囲で実質的に成立済み**
...
6. 原子的な建設・研究・製作Transaction(`unlockFacilityNode()`の「全コスト検証→
   消費→コミット」という既存の原子性は維持。`facility_data_contract.md`が定める
   正式なTransaction ID二重適用防止までは未実装)
7. ユニットページの装備変更(既存実装で成立。Forge分岐の兵種拡張は対象外)
8. 地域攻略による拠点段階と外観変化(`OutpostStage`のEncampment→PioneerOutpost
   のみ実装済み、それ以降の段階・外観変化・共同施設研究は未着手)
