...
状態: **初期6兵種+重装兵(Class・武器・固有能力・スキル)実装済み**

項目1(後半6兵種)は1兵種ずつ実装する方針。重装兵(HeavyInfantry)のClass・武器・固有能力
「重量装甲」・スキル3種(装甲前進/衝撃防御/障害物破砕、実戦効果まで実装)が完了。
加入経路(項目2)は対象外のため、重装兵はまだplayerParty/reserveRosterに登場しない。
残り5兵種(辺境工兵・伝令騎兵・辺境猟兵・旗手・戦闘魔導士)は未着手。
