...
- M9 残り8地域: 灰鉄採石場(第3地域)着手。M9-A(骨格5地点+地点1)・M9-B(地点2
  「砕石段丘」)・M9-C(地点3B「巻上機区画」)・M9-D(地点5「崩落核」、地域ボス
  「灰殻穿岩虫」)完了。地点3A「旧採掘坑」・地点4「灰鉄鉱脈」はゲストユニット
  (護衛対象/操作可能な一時NPC)を前提とし現行エンジンに未実装のため意図的に見送り

直近の未完了(優先度順):

1. M9-E以降 地点3A「旧採掘坑」・地点4「灰鉄鉱脈」(ゲストユニット/護衛脱出
   サブシステムの実装後、または軽量近似での着手を再検討)。灰鉄採石場自体は
   地点5(ボス)まで実コンテンツ化済みのため、次点として黒水低湿地(第4地域)へ
   進む選択肢もある
...
1. **M9-E ゲストユニット/護衛脱出サブシステムの要否検討、または黒水低湿地(第4地域)着手**
   - 地点3A「旧採掘坑」・地点4「灰鉄鉱脈」はいずれも操作可能なゲストユニット
     (HP0で死亡せず撤退)が主目的の前提で、現行エンジンに未実装のまま
   - サブシステムを新設して両地点を仕上げるか、灰鉄採石場を地点5までの内容で
     一旦区切り黒水低湿地へ進むか、次Slice着手前にユーザーと方針を確認する
