...
状態: **項目1・2・3・5は完了(既存のTeam制約付きSecureTile・存在検証付きDefeatUnit・
Any-groupのSuperseded・validateBattleMission()による開始時検証)。項目4は`DestroyObject`/
`SurviveRounds`/`EscapeUnits`/`ProtectUnit`(破壊・防衛・脱出・護衛、いずれも2026-07)が
完了 - 装置操作のみまだ未着手**
...
4. 防衛、護衛、脱出、破壊、装置、Round生存
   ...
   - **完了(2026-07続き)**: 護衛 = `ObjectiveKind::ProtectUnit`を追加。他Kindと違い常に
     ...
     `validateBattleMission()`が拒否する。装置操作はSpawnPoint/Deviceの実挙動が前提のため
     引き続き未着手
5. Mission開始時Definition検証

### M1-C Battle Object最小実装

状態: **「直近の実装順」記載の4項目のうち、データモデル部分、BattleController/UI統合の
...
新設済み。Encounter生成ロジック自体・AI・Boss状態のデータ化、`UnitClass` switch分散の解消は
未着手。M9着手前の必須基盤**
