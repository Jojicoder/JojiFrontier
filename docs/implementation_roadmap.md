...
- M7 12兵種・仲間・会話: 項目1(後半6兵種)完了。項目2(加入経路)は5/6兵種
  ...
  本格攻略に依存)は未着手。項目3(ユニットページと装備共有)は完了Gateの武器重複装備
  防止だけ完了、ユニットページ本体は未着手
...
装備共有)は完了Gateの「同じ武器1本を複数人へ同時装備できない」だけ実装済み、
ユニットページ本体(一覧/詳細/比較パネル/スキル・特性UI一般化)は未着手**。詳細:
...
1. **M7項目3(残り) ユニットページ本体**
   - 一覧・詳細・比較パネル(character_progression.md「ユニットページ」)
   - スキル・特性の装備UIをSpearman専用から全兵種へ一般化
   - 武器重複装備の防止は完了(このSliceで実装済み)
2. **M1-E/M9前基盤整理(残り)**
