...
状態: **初期6兵種+重装兵・辺境工兵・伝令騎兵(Class・武器・固有能力・スキル)実装済み**
...
生成するBattle Object配置を初めて使用)、続いて伝令騎兵(MessengerCavalry)のClass・
武器・固有能力「再移動」・スキル3種(緊急伝令/駆け抜け/救援搬送)が実戦効果まで完了。
再移動(攻撃・スキル・アイテム行動後、生存していれば最大2マス移動して行動終了)は
`BattleController::finishPlayerAction()`を`bool`返却化し、17箇所の呼び出し元すべてに
委譲ガードを追加する形で実装した(既存のどの固有能力とも異なり、あらゆる行動の直後に
割り込む性質のため)。加入経路(項目2)は対象外のため、3兵種ともまだplayerParty/
reserveRosterに登場しない。残り3兵種(辺境猟兵・旗手・戦闘魔導士)は未着手。
