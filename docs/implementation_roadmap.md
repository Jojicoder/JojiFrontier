...
1. M7-2: 辺境工兵・伝令騎兵・旗手の加入経路(安全帰還後の候補登録・二重加入防止・Save)
2. M1-E/M9前基盤整理: 残りRegion/Site/Encounter定義化、RewardRule、Route Graph到達検査、
   Objective達成可能性検査
...
戦闘魔導士)の加入条件配線は今後のSliceで`data/units.json`の`recruits`定義を増やし、
`GameApp::confirmRecruitJoin()`/拠点UI/Save復元が同じ`RecruitDefinition`相当のデータを
参照する形で追加する(次はM7-2として辺境工兵・伝令騎兵・旗手の3体を予定)。
...
2. **M1-E/M9前基盤整理**
   ...
   - Route Graph到達検査
   - Objective達成可能性検査
3. **M7項目3 ユニットページと装備共有**
   - M7-2の次のM7 Slice(ロードマップのM7実装Slice順どおり)
