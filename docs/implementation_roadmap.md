...
- M9 残り8地域: 灰鉄採石場(第3地域)は地点5(ボス)まで完了(M9-A/B/C/D)。地点3A
  「旧採掘坑」・地点4「灰鉄鉱脈」はゲストユニット(護衛対象/操作可能な一時NPC)を
  前提とし現行エンジンに未実装のため意図的に見送り。黒水低湿地(第4地域)着手、
  M9-E(骨格7地点+地点1「灰水の沈み道」)・M9-F(地点2「葦原の分岐」)・M9-G
  (地点3「薬草洲」、`primarySurviveRoundsAlternative`を新規実装)完了。「採取者を
  撤退させない」副目標/敗北条件は、地点3A/4と同じゲストユニット系ギャップとして
  意図的に見送り

直近の未完了(優先度順):

1. M9-H以降 黒水低湿地の残り地点(地点4「樹脂林」から1地点ずつ、M6-B/C方式。
   地点3/4は`AllMembers`分岐で両方必須、地点3は完了済みなので地点4を確保すると
   分岐全体が解決する)
...
`old_barracks`と同じゲストユニット系ギャップ)が無いため意図的に見送った。地点4〜7は
プレースホルダーのまま、M6-B/C方式で1地点ずつ本格化する。残り6地域は未着手
...
1. **M9-H 黒水低湿地 地点4(樹脂林)の実コンテンツ化**
   - `docs/regions/blackwater_lowlands.md`「4. 樹脂林」どおりの内容
   - M9-Eで新設した`resin_grove`プレースホルダーを置き換え
