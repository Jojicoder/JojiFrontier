...
状態: **初期6兵種+重装兵・辺境工兵(Class・武器・固有能力・スキル)実装済み**

項目1(後半6兵種)は1兵種ずつ実装する方針。重装兵(HeavyInfantry)のClass・武器・固有能力
「重量装甲」・スキル3種(装甲前進/衝撃防御/障害物破砕)、続いて辺境工兵(FrontierEngineer)
のClass・武器・固有能力「野戦工作」・スキル3種(野戦補修/瓦礫爆破/即席防壁、実戦効果まで
実装、戦闘中に動的生成するBattle Object配置を初めて使用)が完了。加入経路(項目2)は対象外
のため、どちらもまだplayerParty/reserveRosterに登場しない。残り4兵種(伝令騎兵・辺境猟兵・
旗手・戦闘魔導士)は未着手。
