...
- M7 12兵種・仲間・会話: 項目1(後半6兵種)完了。項目2(加入経路)は5/6兵種
  ...
  装備スキル2枠UI(全兵種)+探索能力表示+比較パネルまで完了、一覧(加入候補非表示は
  既存実装で対応済みと確認)/連携作戦/特性・武器分岐の他兵種拡張は未着手
- M9 残り8地域: 灰鉄採石場(第3地域)着手。M9-A(骨格5地点+地点1)・M9-B(地点2
  「砕石段丘」)・M9-C(地点3B「巻上機区画」)完了。地点3A「旧採掘坑」はゲストユニット
  (護衛対象の一時操作可能NPC)を前提とし現行エンジンに未実装のため意図的に見送り、
  地点4「灰鉄鉱脈」・地点5「崩落核」は仮プレースホルダーのまま(次Slice以降で
  1地点ずつ本格化)

直近の未完了(優先度順):

1. M9-D以降 灰鉄採石場の残り地点(地点4「灰鉄鉱脈」から1地点ずつ、M6-B/C方式。
   地点3A「旧採掘坑」はゲストユニット/護衛脱出サブシステム実装後まで保留)
2. M7項目3(残り) ユニットページ本体(差分プレビュー/連携作戦/特性・武器分岐の
   他兵種一般化)
...
`OperateObject`/`objectPlacementRules`のみで実装、コード変更なし)完了(2026-07)。
地点3A「旧採掘坑」はゲストユニット(護衛対象の一時操作可能NPC、現行エンジンに未実装、
Cinderwatch`old_barracks`「伝令兵脱出」と同じギャップ)前提のため意図的に見送り、
地点4/5は仮プレースホルダー(汎用敵編成)のまま、M6-B/C方式で1地点ずつ本格化する。
残り7地域は未着手
...
1. **M9-D 灰鉄採石場 地点4(灰鉄鉱脈)の実コンテンツ化**
   - `docs/regions/ashiron_quarry.md`「4. 灰鉄鉱脈」どおりの内容
   - M9-Aで新設した`ashiron_vein`プレースホルダーを置き換え
   - 同節で言及されているゲストユニット(戦闘魔導士イリエン)が必要な場合、そのサブ
     システム自体の要否を別途検討する(地点3A同様に見送る可能性あり)
   - 地点3A「旧採掘坑」はゲストユニット/護衛脱出サブシステムが実装されるまで保留
