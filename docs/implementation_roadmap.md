...
状態: **M3-A最小スライス完了。M3-B/C/DおよびM3-Aの残り(Alias/隔離/Snapshot)は未実装**
...
状態: **項目1(Skill Effect Executor)の最小実装が完了、18 Skill中11個(緊急処置・状態治療・制圧射撃・守護処置・隊形維持・足止め突き・奇襲・封鎖強化・標的指定・援護命令・前進命令)
配線済み。項目2〜8は未着手**
...
- 実装済みの4形状と該当Skill:
  ...
  - 他7 Skillの`skillId`はどの形状テーブル・専用分岐にも該当せずno-op(`chooseAttack()`が
    対象0体で何もしないのと同じ扱い)。反応Skill・地形操作・自己移動などに当てはまらない
    ものは、実装時に専用コードか新しい形状テーブルが必要になる
- UI: 戦闘HUDの「スキル」メニューが、今まで暁の衛生兵の生得Heal(`canHeal()`、Tier未満の
  ...
  通りグレーアウト+ホバー理由表示にした
- 回帰テスト: 11 Skillそれぞれの対象判定・効果・Cost消費に加え、`ambush`の未行動限定
  ...
  `clearSkillBuffsAtEnemyPhaseEnd`から独立して切れること、既存分のテストがリファクタ後も
  無変更で通ることを確認
- **未着手**: 残り7 Skill全ての効果実装、Previewと実Resolverの共有、状態異常・地形・
  Boss補正の反映、反応Skill(`counterthrust`)の自動発動、AIによるSkill使用判断。今の5形状
  テーブルでカバーできないSkill(反応・地形操作・自己移動・条件付き効果など)は実装時に
  専用コードか新しい形状テーブルの追加が必要
