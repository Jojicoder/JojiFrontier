...
- M9 残り8地域: 灰鉄採石場(第3地域)は地点5(ボス)まで完了(M9-A/B/C/D)。地点3A
  ...
  M9-E(骨格7地点+地点1「灰水の沈み道」)・M9-F(地点2「葦原の分岐」)完了

直近の未完了(優先度順):

1. M9-G以降 黒水低湿地の残り地点(地点3「薬草洲」から1地点ずつ、M6-B/C方式。
   地点3/4は`AllMembers`分岐で両方必須)
...
Bandit再利用で毒攻撃は未実装のまま)完了(2026-07)。地点3〜7はプレースホルダーの
まま、M6-B/C方式で1地点ずつ本格化する。残り6地域は未着手
...
1. **M9-G 黒水低湿地 地点3(薬草洲)の実コンテンツ化**
   - `docs/regions/blackwater_lowlands.md`「3. 薬草洲」どおりの内容
   - M9-Eで新設した`herb_islet`プレースホルダーを置き換え
   - 地点3A「旧採掘坑」・地点4「灰鉄鉱脈」はゲストユニット/護衛脱出サブシステムが
     実装されるまで引き続き保留
