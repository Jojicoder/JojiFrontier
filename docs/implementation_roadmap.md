...
- M9 残り8地域: 灰鉄採石場(第3地域)着手。M9-A(骨格5地点+地点1「崩落した搬入口」の
  実コンテンツ)完了、地点2〜5は仮プレースホルダーのまま(次Slice以降で1地点ずつ本格化)

直近の未完了(優先度順):

1. M9-B以降 灰鉄採石場の残り地点(地点2「砕石段丘」から1地点ずつ、M6-B/C方式)
...
実コンテンツ)完了(2026-07)。地点2〜5は仮プレースホルダー(汎用敵編成)のまま、
M6-B/C方式で1地点ずつ本格化する。残り7地域は未着手
...
1. **M9-B 灰鉄採石場 地点2(砕石段丘)の実コンテンツ化**
   - `docs/regions/ashiron_quarry.md`「2. 砕石段丘」どおりの探索3択・OR条件付き
     主目的(搬出標識確保 or 敵全滅後に確保)・鉱石箱2個の副目標
   - キャンプIの安全通過(地点1・2を恒久化)
   - M9-Aで新設した`quarry_terrace`プレースホルダーを置き換え
