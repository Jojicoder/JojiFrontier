...
- M7 12兵種・仲間・会話: 項目1(後半6兵種)完了。項目2(加入経路)は4/6兵種
  (重装兵・辺境工兵・伝令騎兵・旗手)完了、残り2兵種(辺境猟兵・戦闘魔導士)は未着手
...
4/6兵種(重装兵・辺境工兵・伝令騎兵・旗手)まで完了**。詳細:
...
だけで加入確定・拠点UI・Save復元がすべて自動的に動く。残り2兵種(辺境猟兵・戦闘魔導士)は
今後のSliceで`recruits`配列への追加+候補付与条件の配線だけで済む見込み。
...
3. **残り2兵種(辺境猟兵・戦闘魔導士)の加入経路**
   - `data/units.json`の`recruits`配列への追加だけで済む見込み(基盤は完成済み)
   - 戦闘魔導士は灰鉄採石場の本格攻略(M9)に依存するため、実質的には辺境猟兵が先
