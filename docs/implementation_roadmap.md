...
1. 燼火峡谷(第7地域)の本格実装(M6-B/C・M9-A〜Y方式)。旧辺境集落(第6地域)は
   ...
   Bandit2体の仮実装のみ)
3. M7項目3(残り) ユニットページ本体(連携作戦)。特性・武器分岐の他兵種一般化は
   2026-07に完了(全12兵種のレシピ・武器データ・UIハードコード解消)。差分プレビュー
   (武器/スキル、「現在」「変更後」「変わる戦術」「失うもの」の4ブロック)も2026-07に
   完了、特性は選択UI自体が無いため対象外(詳細は`implementation_status.md`
   「M7項目3続き 差分プレビュー」参照)。残るのは連携作戦のユニットページ表示・変更UI。
   固有効果のうち、2026-07にコスト「安価〜中程度」の17分岐(条件付きダメージ・回復量
   変化・戦闘1回ガート・引き寄せ・移動コスト軽減・再移動時バフ)を追加接続、
   ノックバック・移動低下・炎上・MOV変化と合わせて計約17+4分岐がエンジン接続済み。
   残り約8分岐(隊形範囲変化・待機トリガーDEF・旗手ステータス変化・再移動後の離脱)は
   未接続(詳細は`implementation_status.md`「M7項目3続き 武器分岐固有効果(基礎〜中程度
   Tier)」参照) - クラフト自体はできるが固有効果が発動しない状態
