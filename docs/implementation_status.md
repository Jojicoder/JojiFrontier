...
## 既知の改善候補: 防具設定レビュー(2026-07-29) → 項目1解消・項目2対応済み(2026-07-29)

状態: 項目1は調査の結果、記録時点で既に解消済みと判明(誤報)。項目2は
ユーザー判断でTier3の数値のみ対応、項目3/4は今回スコープ外として保留。
...
3. **兵種ごとの防具個性が薄い** → 保留(スコープ外)
   全12兵種分の新規数値設計が必要な規模のため、今回は対応しない。

4. **一部Lv1素材のフレーバー再確認** → 保留(スコープ外)
   `armorLevelUpCost()`のprimary素材にも影響する変更のため、今回は
   対応しない。
...
## 既知の改善候補: 装備スキル/武器の役割重複レビュー(2026-07-29)

状態: 未修正メモ。装備スキルと武器分岐はどちらも個性自体はあるが、同一兵種内で
役割が近すぎる組み合わせがあり、装備選択の意味が薄くなる可能性がある。

1. **移動低下攻撃が複数兵種で似ている**
   監視弓兵`suppressing_shot`、槍兵`halting_thrust`、制圧弓`pinning_bow`、
   拘束弓`snare_bow`、辺境猟兵`snare_trap`はいずれも移動低下を中心にした制御。
   武器射程や罠/攻撃の違いはあるが、プレイヤーの印象としては「またMOV低下」に
   見えやすい。改善案: 片方を標的指定、射程制限、地形連動、反応型などへ寄せる。

2. **辺境斥候は武器とスキルの被りが強い**
   `trailblaze`スキルと`trail_blade`、`ambush`スキルと`ambush_blade`が
   ほぼ同じ方向を向いている。これは専門化ビルドとしては分かりやすいが、
   「スキルでできることを武器でもう一度やる」印象になりやすい。
   改善案: 武器側は常時/小効果、スキル側は一度きりの大効果に寄せるなど、
   同系統でも役割差を明確にする。

3. **待機防御系が近い**
   古参守備兵`immovable_stance`、重装兵`brace_for_impact`、防壁槌`bulwark_maul`、
   巡回槍`patrol_lance`は「待機するとDEF系強化」という読み味が近い。
   強制移動無効、次行動まで、次Player Phaseまで、など差はあるが、UI上で
   違いを強く伝えないと選択理由が埋もれる。

4. **移動支援が多い**
   行軍隊長`advance_order`、伝令騎兵`urgent_dispatch`、旗手`marching_rhythm`、
   行軍杖`march_staff`はいずれも味方MOV上昇。単体/範囲/回復付随/値の違いはあるが、
   複数持ち込むと役割が重なりやすい。改善案: 旗手は範囲、伝令騎兵は位置入れ替えや
   搬送、行軍隊長は陣形維持の延長、杖は回復支援という棲み分けをUIと数値で強める。

5. **武器分岐は全体としてスキルより個性が強い**
   武器側は射程変更、MOV補正、ノックバック、引き寄せ、旗オーラ変更、杖の回復副効果、
   焦点具の範囲/炎上など、選んだ瞬間にプレイスタイルが変わるものが多い。
   ただし支援武器のLv成長がmight中心で伝わりにくい問題は別途残る。
...
## 既知の改善候補: ゲームバランスレビュー(2026-07-29)
...
5. **戦闘魔導士は火力が突出しやすいが、装備スキル再装備導線が弱い**
   MAG9+魔導焦点具威力6で、低RES敵への実ダメージが非常に高い。Lv5や
   `war_focus`まで入ると通常敵の耐久を追い越す可能性がある。一方で
   `requiredTrainingNodeIdFor(BattleMage)`が空文字列のため、通常の
   `equipSkillForUnit()`では再装備できない既知問題があり、強さ以前に導線が不安定。
   改善案: 魔導士は高火力を維持するなら耐久/配置/燃費/素材で明確に制約し、
   専用解放ノードまたはDiscovery条件でスキル変更導線を整える。
