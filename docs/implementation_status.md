...
## 既知の改善候補: 装備差分/帰還UIの追加文言レビュー(2026-07-29) → 記録のみ

状態: ユーザー指摘の記録のみ。既存の日本語文言修正後も、まだ分かりにくい箇所が残る。

1. **「差分プレビュー」という日本語が分かりにくい**
   装備画面の比較欄に「差分プレビュー」と表示されるが、ゲームUIとして何を
   見せている欄なのか直感的ではない。改善案: 「装備変更の確認」、
   「変更内容」、「装備比較」など、プレイヤーの行動に近い見出しへ変更する。

2. **差分欄の詳細がまだ足りない**
   「能力変化 RES+1」「外れる効果 (なし)」だけでは、選んだ武器/防具/スキルが
   実際に何を変えるのか判断しづらい。特にスキルでは、対象、射程、使用条件、
   回数制限、効果が切れるタイミング、外れるスキルの効果が同じ欄で読めない。
   改善案: 差分欄に「現在の効果」「変更後の効果」「変わる数値」「外れる効果」を
   短く分けて表示し、詳細Tooltipへの誘導も入れる。

3. **「もう一度帰還する」の意味が分かりにくい**
   倉庫整理画面の下部に「もう一度帰還する」と表示されるが、既に帰還処理中なのか、
   倉庫整理後に帰還確定を再試行するのか、プレイヤーには分かりにくい。
   改善案: ボタン名を「整理して帰還を確定」「帰還処理を再試行」
   「戦利品を確定する」など、何が起きるか分かる文言に変更する。
   説明文も「倉庫整理後、入りきらなかった戦利品をもう一度確定します」のように
   目的を補足する。
